...
The zoos need a list of all their animal's names (names only, not scientific) converted to lower case.  Create a new array named lowerCase 
and map over each name to convert them all to lower case.  Log the resut.

*/

const lowerCase = [];
let alllowerCase = zooAnimals.map((person) => {
  return person.animal_name.lowerCase();
});
...
let totalPop = zooAnimals.reduce((pop, allPop) => {
  return pop + allPop.population;
}, 0)
